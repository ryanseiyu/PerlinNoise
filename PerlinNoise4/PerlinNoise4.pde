...
void draw() {
  ...
  for (int y = 0; y < rows; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      int index = x + y * cols;
      angle = noise(xoff, yoff, zoff) * TWO_PI * 2; //Last value can be modified to add more chaotic movement
      //flowField[index] = vector;
      PVector vector = new PVector(0, 0).fromAngle(angle);
      vector.setMag(0.08); //Force magnitude.
      flowField[index] = vector;
      xoff += inc;
      /**stroke(0, 50);
      strokeWeight(1);
      ...
      line(0, 0, scl, 0);
      popMatrix(); **/ //Block to show the flow field.
      //rect(x * scl, y * scl, scl, scl);
    }
    yoff += inc;
    zoff += 0.0003; //Movement of vectors.
  }
